...
#| echo: false
#| tbl-cap: "Beta Regression Results All -> Urban Land Cover Change"
...
model2.1_residuals <- residuals(AllUrban_turnover_model1_beta_regression)

# Add residuals to df
turnover_all_urban_15km$residuals <- model2.1_residuals

# Get summary
model_summary <- tidy(AllUrban_turnover_model1_beta_regression,
                      conf.int = TRUE)
...
model_glance <- glance(AllUrban_turnover_model1_beta_regression)
...
# Display all_urban_15km_local_morans_I
all_urban_15km_local_morans_I
...
# Load model
load(here("data", "models", "exploratory", "AllUrban_turnover_model2_gls_log.RData"))

# Extract correlation structure parameters
print(AllUrban_turnover_model2_gls_log$modelStruct$corStruct)

# Get range parameter
range_param <- coef(AllUrban_turnover_model2_gls_log$modelStruct$corStruct, unconstrained = FALSE)
```
...
# Extract residuals
residuals_gls <- residuals(AllUrban_turnover_model2_gls_log, type = "normalized")

# Get the model frame to ensure we're using the exact same data
model_data <- getData(AllUrban_turnover_model2_gls_log)

# Basic residual plots
par(mfrow = c(2, 2))

# Residuals vs fitted
plot(fitted(AllUrban_turnover_model2_gls_log), residuals_gls,
...
#| echo: false
#| tbl-cap: "GLS Model Coefficients All → Urban Effects on Biodiversity Turnover"
...
cat("- Formula:", deparse(AllUrban_turnover_model2_gls_log$call$model), "\n")
...
n_obs <- tryCatch({model_summary$dims$N
  ...
  nobs(AllUrban_turnover_model2_gls_log)
})
...
fit_stats <- data.frame(Statistic = c("AIC", "BIC", "Log-likelihood", "Residual Std Error"),
                        Value = c(round(AIC(AllUrban_turnover_model2_gls_log), 2),
                                  round(BIC(AllUrban_turnover_model2_gls_log), 2), 
                                  round(as.numeric(logLik(AllUrban_turnover_model2_gls_log)), 2),
...
# Jaccard's Dissimilarity Index
plant_jdi_hist <- ggplot(plants_turnover_all_urban_15km_coords_time, aes(x = JDI)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white", alpha = 0.7) +
  facet_wrap(~ lc_time_period) +
  labs(x = "Jaccard Dissimilarity Index",
       y = "Count") +
...
# Correlation matrix for numeric variables
plant_numeric_vars <- plants_turnover_all_urban_15km_coords_time |>
  select(JDI, all_to_urban, urban_no_change, 
         intersection_size, union_size, 
         total_spp_before, total_spp_after,
         total_occ_before, total_occ_after,
...
### Trial1: GLS with logged recorder effort
...
# Load model
load(here("data", "models", "exploratory", "plants_AllUrban_model1_gls.RData"))
...
# Model information
cat("Model Specifications:\n")
cat("- Formula:", deparse(plants_AllUrban_model1_gls$call$model), "\n")
cat("- Correlation Structure: Exponential spatial correlation\n") 
cat("- Method: REML\n")
...
n_obs <- tryCatch({model_summary$dims$N
  ...
  nobs(plants_AllUrban_model1_gls)
})
...
# Model fit statistics
fit_stats <- data.frame(Statistic = c("AIC", "BIC", "Log-likelihood", "Residual Std Error"),
                        Value = c(round(AIC(plants_AllUrban_model1_gls), 2),
                                  round(BIC(plants_AllUrban_model1_gls), 2), 
                                  round(as.numeric(logLik(plants_AllUrban_model1_gls)), 2),
                                  round(model_summary$sigma, 4)))
knitr::kable(fit_stats, col.names = c("Statistic", "Value"))
```
...
# Jaccard's Dissimilarity Index
bird_jdi_hist <- ggplot(birds_turnover_all_urban_15km_coords_time, aes(x = JDI)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white", alpha = 0.7) +
  facet_wrap(~ lc_time_period) +
  labs(x = "Jaccard Dissimilarity Index",
       y = "Count") +
...
# Correlation matrix for numeric variables
bird_numeric_vars <- birds_turnover_all_urban_15km_coords_time |>
  select(JDI, all_to_urban, urban_no_change, 
         intersection_size, union_size, 
         total_spp_before, total_spp_after,
         total_occ_before, total_occ_after,
...
# Load model
load(here("data", "models", "exploratory", "birds_AllUrban_model1_gls.RData"))
...
#| echo: false
#| tbl-cap: "GLS Model Coefficients All -> Urban Effects on Birds Turnover"

# Extract model summary
model_summary_birds <- summary(birds_AllUrban_model1_gls)
...
# Model information
cat("Model Specifications:\n")
cat("- Formula:", deparse(birds_AllUrban_model1_gls$call$model), "\n")
cat("- Correlation Structure: Exponential spatial correlation\n") 
cat("- Method: REML\n")
...
n_obs <- tryCatch({model_summary_birds$dims$N
  ...
  nobs(birds_AllUrban_model1_gls)
})
...
# Model fit statistics
fit_stats <- data.frame(Statistic = c("AIC", "BIC", "Log-likelihood", "Residual Std Error"),
                        Value = c(round(AIC(birds_AllUrban_model1_gls), 2),
                                  round(BIC(birds_AllUrban_model1_gls), 2), 
                                  round(as.numeric(logLik(birds_AllUrban_model1_gls)), 2),

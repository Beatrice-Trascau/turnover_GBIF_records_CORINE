...
model_summary <- summary(model1_gls)
...
cat("- Formula:", deparse(model1_gls$call$model), "\n")
...
fit_stats <- data.frame(Statistic = c("AIC", "BIC", "Log-likelihood", "Residual Std Error"),
                        Value = c(round(AIC(model1_gls), 2),
                                  round(BIC(model1_gls), 2), 
                                  round(as.numeric(logLik(model1_gls)), 2),
...
# Load model
load(here("data", "models", "gls_model3_plant_occurrences_turnover_forest_tws_results.RData"))
...
1.  Residuals vs Fitted
    -   There is a clear curve, with residuals becoming more negative with increasing fitted values, which suggests a non-linear relationship
2.  QQ plot
...
3.  Residuals vs Forest to TWS
    ...
    -   No clear trend
4.  Residuals vs Recorder Effort
    -   Slight positive skew in residuals with increasing effort
    -   Wider spread at higher effort
    -   Could indicate some heteroscedasticity

The issue of non-linearity is the most concerning one, and therefore we will try a GAM and see if it is any better.

### (Final)Option 2: GAM with spatial smoother
...
# Load model
load(here("data", "models", "gam_model4_plant_occurrences_turnover_forest_tws_results.RData"))
...
The diagnostic plots reveal some mild deviances from expectations, but no major issues. Since there is some curvature to the relationship between residuals and Forest to TWS, I will also try a model that fits a smoother around that variable.
...
# Load model
load(here("data", "models", "gam_model5_plant_occurrences_turnover_forest_tws_results.RData"))
...
```{r bird gls}
#| label: fig-bird-gls-output
#| echo: false
#| message: false
#| fig-width: 12
#| fig-height: 8
#| fig-cap: "Diagnostic plots for GLS model for bird occurrences"

# Load model
load(here("data", "models", "gls_model6_birds_occurrences_turnover_forest_tws_results.RData"))
...
any(!is.finite(birds_turnover_forest_tws_15km_coords_time$recorder_effort))
...
### Option 2: GAM with spatial smoother
...
# Load model
load(here("data", "models", "gam_model7_birds_occurrences_turnover_forest_tws_results.RData"))
...
Although this looks better than GLS and does not reveal any serious issues, the validation using DHARMa revealed that there are still some serious issues to be considered/acknowledged.

```{r birds DHARMa}
#| label: fig-birds-gam-DHARMa
#| echo: false
#| message: false
#| fig-width: 12
#| fig-height: 8
#| fig-cap: "DHARMa diagnostic plots for GAM model for bird occurrences"

# Simulate residuals
res <- simulateResiduals(fittedModel = birds_model7_gam, plot = TRUE)
```

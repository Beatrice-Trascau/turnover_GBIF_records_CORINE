...
# 4.2_turnover_plants_land_cover
...
save(turnover_2000_2006_lc,
     file = here("data", "derived_data", "plants_turnover_2000_2006_all_data.rda"))
...
save(turnover_2006_2012_lc,
     file = here("data", "derived_data", "plants_turnover_2006_2012_all_data.rda"))
...
save(turnover_2012_2018_lc,
     file = here("data", "derived_data", "plants_turnover_2012_2018_all_data.rda"))
...
nrow(vascular_plants_all_periods_turnover_all_land_cover_chanegs_15km)
unique(vascular_plants_all_periods_turnover_all_land_cover_chanegs_15km$lc_time_period)

# Merge climate data with the existing turnover dataframe
vascular_plants_turnover_with_climate <- vascular_plants_all_periods_turnover_all_land_cover_chanegs_15km |>
  left_join(all_climate_data, by = c("cell_ID", "lc_time_period"))

# Check it all went ok
nrow(vascular_plants_turnover_with_climate)
cat("Climate variables added:", sum(c("temp_before", "temp_after", "precip_before", 
                                      "precip_after", "temp_change", "precip_change") %in% 
                                      names(vascular_plants_turnover_with_climate)), "/ 6\n")

## 8.5. Quality checks ---------------------------------------------------------

# Check for missing climate data
climate_na_summary <- vascular_plants_turnover_with_climate |>
...
cat("Temperature before (°C):", round(range(vascular_plants_turnover_with_climate$temp_before, na.rm = TRUE), 2), "\n")
cat("Temperature after (°C):", round(range(vascular_plants_turnover_with_climate$temp_after, na.rm = TRUE), 2), "\n")
cat("Temperature change (°C):", round(range(vascular_plants_turnover_with_climate$temp_change, na.rm = TRUE), 2), "\n")
cat("Precipitation before (mm/yr):", round(range(vascular_plants_turnover_with_climate$precip_before, na.rm = TRUE), 0), "\n")
cat("Precipitation after (mm/yr):", round(range(vascular_plants_turnover_with_climate$precip_after, na.rm = TRUE), 0), "\n")
cat("Precipitation change (mm/yr):", round(range(vascular_plants_turnover_with_climate$precip_change, na.rm = TRUE), 0), "\n")

# Save dataframe of turnove + the new climate data
save(vascular_plants_turnover_with_climate,
     file = here("data", "derived_data", 
                 "vascular_plants_turnover_all_land_cover_climate_15km.rda"))

# One last check!
cat("\n✅ Climate integration complete!\n")
cat("Enhanced dataset saved with", ncol(vascular_plants_turnover_with_climate), "variables\n")
...
for(var in climate_vars) {
  if(var %in% names(vascular_plants_turnover_with_climate)) {
    cat(" ✓", var, "\n")
  }
}

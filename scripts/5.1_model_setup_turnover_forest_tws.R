...
saveRDS(C1, 
        here("data", "models", "exploratory", 
             "bayes_birds_FTWS_C1_ordbeta_const_spatial.rds"))
...
saveRDS(C2, 
        here("data", "models", "exploratory", 
             "bayes_birds_FTWS_C2_ordbeta_disp_spatial.rds"))
...
for (this_id in model_specs$id) {
  ...
  saveRDS(fit, here("data", "models", "final",
                    paste0("bayes_turnover_", this_id, "_ordbeta_disp_spatial.rds")))
}
...
for (this_id in model_specs$id) {
  ...
  resid_full <- dat$beta_jtu - yhat_full
  ...
  for (p in cont_preds) {
    ...
    # overlay A: raw observed points (raw predictor vs observed turnover)
    point_rows[[length(point_rows) + 1]] <- tibble::tibble(
      id = this_id, group = spec$group, transition = spec$transition,
      predictor = p, x_value = raw, value = dat$beta_jtu)
...
# Plot the same figure as above but with the raw data points added
...
### 5.2.2. Overaly B: parital residuals ----------------------------------------
